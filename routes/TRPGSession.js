...
router.post('/TRPGCreateSession',async function (req,res) {
    ...
    if(error) return res.status(400).redirect('/trpgsession/create'),req.app.io.emit('alert',error.details[0].message);

    //check if the session is already exist
    const sessionExist = await Session.findOne({name:req.body.name});
    if (sessionExist) return res.status(400).redirect('/trpgsession/create'),req.app.io.emit('alert','此名稱已存在');
...
router.post('/TRPGJoinSession',async function (req,res) {
    //check if the format is correct
    const {error}= sessionValidation(req.body);
    if(error) return res.status(400).redirect('/trpgsession/join'),req.app.io.emit('alert',error.details[0].message);
    ...
    if (!session) return res.status(400).redirect('/trpgsession/join'),req.app.io.emit('alert','此團務不存在');
    //check the password
    if (req.body.password !== session.password ) return res.status(400).redirect('/joinsession'),req.app.io.emit('alert', '密碼錯誤');

    if (playerExist) return res.status(400).redirect('/trpgsession/join'),req.app.io.emit('alert','你已加入此團務');
    try{
